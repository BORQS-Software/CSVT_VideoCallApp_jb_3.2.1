...
public class VTCallReceiver extends BroadcastReceiver {
	...
	void AddBlockNumberToCallLog(String strAddr)
	{
		...
		final int _duration =  0 ;// in seconds
	        ...
	        try
	        {
//	         Calls.addCall( ci, mCtx, _number, _presentation , _type,_start,_duration , false); 
		      Calls.addCall( ci, mCtx, _number, _presentation , _type,_start,_duration); //Gary ,0); 
	        }
	        catch (Exception e) {
				Log.e(TAG, "Exception occurs while add block Number to CallLog." + e.getMessage());}
